...
import { setOnlinePersons } from "../store/Slices/onlinePersonsSlice.ts";
...
function useWebSocket() {
  ...
  const onConnected = (stompClient: Stomp.Client, username: string) => {
    ...
    stompClient.subscribe("/topic/online", onOnlinePersons);
    stompClient.send(
      "/app/chat.addUser",
      {},
      JSON.stringify({
  ...
  const onOnlinePersons = (payload: Stomp.Message) => {
    dispatch(setOnlinePersons(JSON.parse(payload.body)));
  };
